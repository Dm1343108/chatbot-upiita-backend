...
// Si estamos en Render (o cualquier servidor) y nos pasan el JSON por env:
if (process.env.GOOGLE_APPLICATION_CREDENTIALS_JSON) {
  const credPath = path.join(process.cwd(), "df-service-account.json");
  fs.writeFileSync(credPath, process.env.GOOGLE_APPLICATION_CREDENTIALS_JSON);
  process.env.GOOGLE_APPLICATION_CREDENTIALS = credPath;
}

const MAPAS_DIR = path.resolve(__dirname, ".", "mapas");
  ...
  for (const base of basenames) {
    for (const ext of exts) {
      ...
      if (fs.existsSync(full)) {
        return `${PUBLIC_BASE_URL}/mapas/${base}${ext}`;
      }
    }
  }
